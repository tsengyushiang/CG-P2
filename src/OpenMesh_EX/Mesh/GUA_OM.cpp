...
namespace OMP
{
	...
	void Model::Render_solid()
	{
		...
		{
			std::cout << f_it.handle().idx() << std::endl;
...
{	
		...
		{		
			glColor4f(property(R, f_it.handle()),
				property(G, f_it.handle()),
				property(B, f_it.handle()),	
				property(A, f_it.handle()));		

			glBindTexture(GL_TEXTURE_2D, property(texID, f_it.handle()));
			glBegin(GL_TRIANGLES);					

			auto fv_it = fv_iter(f_it);

			glNormal3dv(normal(fv_it.handle()).data());
			glTexCoord2d(property(x1, f_it.handle()), property(y1, f_it.handle()));
			glVertex3dv(point(fv_it.handle()).data());

			++fv_it;

			glNormal3dv(normal(fv_it.handle()).data());
			glTexCoord2d(property(x2, f_it.handle()), property(y2, f_it.handle()));
			glVertex3dv(point(fv_it.handle()).data());

			++fv_it;

			glNormal3dv(normal(fv_it.handle()).data());
			glTexCoord2d(property(x3, f_it.handle()), property(y3, f_it.handle()));
			glVertex3dv(point(fv_it.handle()).data());
